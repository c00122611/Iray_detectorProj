...
#define SUPPORT_MONOTONIC_CLOCK
//#endif
#endif
#include "winevent.h"
...
int UnlockedWaitForEvent(HEVENT event, uint64_t milliseconds)
{
    ...
    if (!event->State)
    {
        ...
        if (milliseconds != (uint64_t) -1)
        {
            ...
            uint64_t nanoseconds = ((uint64_t)ts_now.tv_sec) * 1000 * 1000 * 1000 + ts_now.tv_nsec + milliseconds * 1000 * 1000;
#else
            timeval tv;
